...
WF_TEST_MONTHS  = 12     # months to predict before re-fitting
...
def _fetch_wpi_2223_series():
    """
    Fetches the NEW 2022-23 base series WPI — All Commodities (Base 2022-23=100)
    from indx_download_2223/wpi_monthly_index_{YYYYMM}.xlsx on eaindustry.nic.in.
    Returns a monthly WPI index pd.Series, or None on failure.
    """
    import io, urllib.request
    from datetime import datetime
    from dateutil.relativedelta import relativedelta

    base_url = 'https://eaindustry.nic.in/indx_download_2223/wpi_monthly_index_{}.xlsx'
    now = datetime.now()

    # Auto-discover latest published file (walk back up to 12 months)
    for months_back in range(13):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as resp:
                content = resp.read()
                if len(content) > 5000:  # valid xlsx (not a 404 HTML page)
                    df_xls = pd.read_excel(io.BytesIO(content), sheet_name=0, header=None, engine='openpyxl')
                    header_row = df_xls.iloc[0]
                    all_comm_row = df_xls.iloc[1]  # "All Commodities" row

                    # Column headers are like 'Apr-23', 'May-23', ..., 'Aug-26'
                    month_map = {'Jan':1,'Feb':2,'Mar':3,'Apr':4,'May':5,'Jun':6,
                                 'Jul':7,'Aug':8,'Sep':9,'Oct':10,'Nov':11,'Dec':12}
                    dates, values = [], []
                    for c in range(4, df_xls.shape[1]):  # data starts at column 4
                        col_name = str(header_row[c]).strip()
                    ...
                    if dates:
                        wpi_index = pd.Series(values, index=dates, name='WPI_2223').sort_index()
                        print(f"  \u2713 WPI 2022-23 series: {len(wpi_index)} months "
                              f"({wpi_index.index[0].strftime('%Y-%m')} \u2192 {wpi_index.index[-1].strftime('%Y-%m')}) "
                              f"[source: wpi_monthly_index_{label}.xlsx]")
                        return wpi_index
        except Exception:
            continue
    return None
...
def _fetch_wpi_1112_series():
    """
    Fetches the LEGACY 2011-12 base series WPI — All Commodities (Base 2011-12=100)
    from indx_download_1112/monthly_index_{YYYYMM}.xls on eaindustry.nic.in.
    Returns a monthly WPI index pd.Series, or None on failure.
    """
    import io, urllib.request
    from datetime import datetime
    ...
    for months_back in range(13):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as resp:
                content = resp.read()
                if len(content) > 2000:
                    df_xls = pd.read_excel(io.BytesIO(content), sheet_name=0, header=None, engine='xlrd')
                    header_row = df_xls.iloc[0]
                    ...
                    if dates:
                        wpi_index = pd.Series(values, index=dates, name='WPI_1112').sort_index()
                        print(f"  \u2713 WPI 2011-12 series: {len(wpi_index)} months "
                              f"({wpi_index.index[0].strftime('%Y-%m')} \u2192 {wpi_index.index[-1].strftime('%Y-%m')}) "
                              f"[source: monthly_index_{label}.xls]")
                        return wpi_index
        except Exception:
            continue
    return None
...
def fetch_india_wpi():
    """
    Fetches official India Wholesale Price Index (WPI) — All Commodities from
    the Office of the Economic Adviser, DPIIT, GoI (eaindustry.nic.in).

    PRIMARY: Uses the actively-updated 2022-23 base series (Base 2022-23=100).
    FALLBACK: For historical months not covered by the 2022-23 series, backfills
              with the legacy 2011-12 base series (Base 2011-12=100).

    Since the final output is Year-over-Year percentage change (base-invariant),
    no linking factor is needed — both series produce comparable YoY inflation rates.

    Auto-discovers the latest available monthly file for each series.
    No synthetic, calibrated, or simulated data is used.
    """
    # ── Step 1: Fetch primary 2022-23 series ──
    wpi_new = _fetch_wpi_2223_series()

    # ── Step 2: Fetch legacy 2011-12 series (for historical backfill) ──
    wpi_old = _fetch_wpi_1112_series()

    if wpi_new is None and wpi_old is None:
        print("  [warn] WPI: Could not fetch either series from eaindustry.nic.in — using fallback value")
        return None

    # ── Step 3: Compute YoY % change for each available series ──
    yoy_new = None
    yoy_old = None

    if wpi_new is not None and len(wpi_new) > 12:
        yoy_new = (wpi_new.pct_change(12) * 100).dropna()
        yoy_new.name = 'WPI_YoY'

    if wpi_old is not None and len(wpi_old) > 12:
        yoy_old = (wpi_old.pct_change(12) * 100).dropna()
        yoy_old.name = 'WPI_YoY'

    # ── Step 4: Splice — prefer 2022-23 series, backfill older months from 2011-12 ──
    if yoy_new is not None and yoy_old is not None:
        # Use 2011-12 series for months BEFORE the 2022-23 YoY series starts
        cutoff = yoy_new.index.min()
        old_backfill = yoy_old[yoy_old.index < cutoff]
        wpi_yoy = pd.concat([old_backfill, yoy_new]).sort_index()
        wpi_yoy.name = 'WPI_YoY'
        series_label = f"2022-23 (primary) + 2011-12 (backfill before {cutoff.strftime('%Y-%m')})"
    elif yoy_new is not None:
        wpi_yoy = yoy_new
        series_label = "2022-23 only"
    else:
        wpi_yoy = yoy_old
        series_label = "2011-12 only (2022-23 unavailable)"

    if wpi_yoy is None or len(wpi_yoy) == 0:
        print("  [warn] WPI: Insufficient data to compute YoY — using fallback value")
        return None

    print(f"  \u2713 WPI (Spliced): {len(wpi_yoy)} months ({wpi_yoy.index[0].strftime('%Y-%m')} \u2192 "
...
def fetch_live_market_data(start_date="2015-01-01"):
    ...
    if macro is not None:
        # Apply publication lag: Indian macro releases are delayed ~6 weeks.
...
def fetch_live_sector_data(start_date="2015-01-01"):
    ...
    # ── Automated Bad-Tick Outlier Filter & Bluechip Index Backfill ──
    # Single-day returns exceeding ±25% for an ETF indicate data feed corruption
    # (e.g. Yahoo Finance 10:1 decimal misplaced ticks) and are automatically sanitized
    # using the underlying sector index proxy.

    # 1. Bank ETF (sanitized with ^NSEBANK for missing data or Yahoo decimal anomalies)
    bank_etf = np.log(raw['BANKBEES.NS'] / raw['BANKBEES.NS'].shift(1))
    bank_idx = np.log(raw['^NSEBANK'] / raw['^NSEBANK'].shift(1))
    bank_etf = bank_etf.where(bank_etf.abs() <= 0.25, bank_idx)
    df_sec['BANKBEES']   = bank_etf.combine_first(bank_idx)

    # 2. IT ETF (backfilled with ^CNXIT prior to 2020 + bad tick filter)
    it_etf = np.log(raw['ITBEES.NS'] / raw['ITBEES.NS'].shift(1))
    it_idx = np.log(raw['^CNXIT'] / raw['^CNXIT'].shift(1))
    it_etf = it_etf.where(it_etf.abs() <= 0.25, it_idx)
    df_sec['ITBEES']     = it_etf.combine_first(it_idx)

    # 3. Pharma ETF (backfilled with ^CNXPHARMA prior to 2021 + bad tick filter)
    ph_etf = np.log(raw['PHARMABEES.NS'] / raw['PHARMABEES.NS'].shift(1))
    ph_idx = np.log(raw['^CNXPHARMA'] / raw['^CNXPHARMA'].shift(1))
    ph_etf = ph_etf.where(ph_etf.abs() <= 0.25, ph_idx)
    df_sec['PHARMABEES'] = ph_etf.combine_first(ph_idx)

    # 4. Auto ETF (backfilled with Maruti + M&M prior to 2022 + bad tick filter)
    ...
    auto_etf = auto_etf.where(auto_etf.abs() <= 0.25, auto_px)
    df_sec['AUTOBEES']   = auto_etf.combine_first(auto_px)

    # 5. Metal ETF (backfilled with Tata Steel + JSW Steel prior to 2024 + bad tick filter)
    ...
    met_etf = met_etf.where(met_etf.abs() <= 0.25, met_px)
    df_sec['METALIETF']  = met_etf.combine_first(met_px)

    # 6. Realty ETF (backfilled with DLF prior to 2024 + bad tick filter)
    rea_etf = np.log(raw['MOREALTY.NS'] / raw['MOREALTY.NS'].shift(1))
    rea_px  = np.log(raw['DLF.NS'] / raw['DLF.NS'].shift(1))
    rea_etf = rea_etf.where(rea_etf.abs() <= 0.25, rea_px)
    df_sec['MOREALTY']   = rea_etf.combine_first(rea_px)

    # 7. Energy ETF (CPSEETF has continuous live history since 2015 + bad tick filter)
    cpse_etf = np.log(raw['CPSEETF.NS'] / raw['CPSEETF.NS'].shift(1))
    df_sec['CPSEETF']    = cpse_etf.where(cpse_etf.abs() <= 0.25, np.nan).ffill().bfill()

    # 8. Infra ETF (backfilled with L&T prior to 2018 + bad tick filter)
    inf_etf = np.log(raw['INFRABEES.NS'] / raw['INFRABEES.NS'].shift(1))
    inf_px  = np.log(raw['LT.NS'] / raw['LT.NS'].shift(1))
    inf_etf = inf_etf.where(inf_etf.abs() <= 0.25, inf_px)
...
def regularize_transmat(transmat, alpha=1.0, max_diag=0.97, min_offdiag=0.005):
    ...
    The Baum-Welch MLE can produce degenerate transition matrices where
    P(i->i) = 1.0 (absorbing states). This function applies a symmetric
    Dirichlet prior (conjugate prior for the multinomial/transition rows)
    and then clips diagonals to enforce ergodicity.

    Math:
        P_smoothed[i,j] = (N_eff * P_mle[i,j] + alpha) / (N_eff + K*alpha)

    Then diagonal values are clamped to [0, max_diag] and off-diagonals
    are floored at min_offdiag to guarantee all states remain communicating.

    Parameters
    ----------
    transmat : ndarray (K, K)
        MLE transition matrix from Baum-Welch.
    alpha : float
        Dirichlet concentration parameter. Higher = stronger smoothing.
        alpha=1.0 corresponds to a uniform (Laplace-like) prior.
    max_diag : float
        Maximum allowed self-transition probability (default 0.98).
    min_offdiag : float
        Minimum off-diagonal probability floor (default 0.005).

    Returns
    -------
    reg : ndarray (K, K)
        Regularized, row-stochastic transition matrix.
    """
    K = transmat.shape[0]
    N_eff = 500  # effective sample size for prior blending

    reg = np.copy(transmat)

    # Step 1: Dirichlet prior blending
    for i in range(K):
        row = transmat[i]
        reg[i] = (N_eff * row + alpha) / (N_eff + K * alpha)

    # Step 2: Clip diagonals to prevent absorbing states
    for i in range(K):
        if reg[i, i] > max_diag:
            excess = reg[i, i] - max_diag
            reg[i, i] = max_diag
            # Redistribute excess equally to off-diagonal entries
            off_diag_mask = np.ones(K, dtype=bool)
            off_diag_mask[i] = False
            reg[i, off_diag_mask] += excess / (K - 1)

    # Step 3: Floor off-diagonals to guarantee ergodicity
    for i in range(K):
        for j in range(K):
            if i != j and reg[i, j] < min_offdiag:
                deficit = min_offdiag - reg[i, j]
                reg[i, j] = min_offdiag
                reg[i, i] -= deficit  # take from diagonal

    # Step 4: Re-normalise rows to be exactly stochastic
    for i in range(K):
        reg[i] = np.maximum(reg[i], 0)
        reg[i] /= reg[i].sum()
...
def train_hmm(X_scaled, n_states=4, n_iter=500, n_init=20):
    ...
    Applies Dirichlet prior regularization post-training to prevent absorbing states.
    """
    best_ll, best_model = -np.inf, None
    ...
    # ── Dirichlet Prior Regularization ──────────────────────────────
    raw_diag = np.diag(best_model.transmat_).copy()
    best_model.transmat_ = regularize_transmat(best_model.transmat_)
    reg_diag = np.diag(best_model.transmat_)
    ...
    print(f"  Transition matrix regularized (Dirichlet prior α=1.0, max_diag=0.98):")
    for s in range(best_model.n_components):
        flag = " ⚠ ABSORBING" if raw_diag[s] >= 0.999 else ""
        print(f"    State {s}: P(i→i) {raw_diag[s]:.4f} → {reg_diag[s]:.4f}{flag}")
...
def smooth_regimes_causal(regime_series, min_hold=MIN_HOLD_DAYS):
    """
    Causal anti-whipsaw filter (no lookahead). Once you enter a regime, you must
    hold it for at least `min_hold` trading days before any switch is permitted.
    Each day's output depends ONLY on past and present observations.

    Unlike the standard smooth_regimes() which looks ahead to measure run length,
    this filter enforces a forward-only lockout period.
    """
    ...
    for i in range(1, len(regime_series)):
        if regime_series[i] != current:
            if hold_count >= min_hold:
                # Enough days held → allow the switch
                current = regime_series[i]
                hold_count = 1
            else:
                # Not enough days yet → stay in current regime (lockout)
                hold_count += 1
        else:
            hold_count += 1
...
def online_forward_decode(model, X_scaled):
    """
    Day-by-day causal forward filtering. Each day's regime is determined using
    ONLY observations up to and including that day — no backward pass, no future
    data.  This replaces batch Viterbi decoding for walk-forward OOS evaluation.

    Uses the HMM's learned parameters (startprob_, transmat_, emission model) to
    compute α_t (forward variable) sequentially.

    Returns:
        np.ndarray of most-likely state indices, shape (n_samples,)
    """
    import numpy as np
    ...
    # Day 0: use start probabilities + first observation
    log_alpha = log_startprob + framelogprob[0]
    states[0] = np.argmax(log_alpha)

    for t in range(1, n_samples):
        # Forward step: α_t(i) = [Σ_j α_{t-1}(j) · A(j→i)] · B(i, X_t)
        log_alpha = np.logaddexp.reduce(
            log_alpha[:, None] + log_transmat, axis=0
        ) + framelogprob[t]
        # Normalize in log-space to prevent underflow accumulation
...
def walk_forward_validation(feat, df, df_sec=None, learned_sector_mix=None, train_years=3, test_months=2, n_states=4):
    ...
    Leakage fixes applied:
      1. Per-fold sector mix: weights are optimised on the EXPANDING TRAINING SET only.
         The `learned_sector_mix` parameter is IGNORED (kept for API compat).
      2. Online forward decode: OOS regime labels are produced day-by-day using
         the forward algorithm — no backward pass, no batch Viterbi.
      3. Causal anti-whipsaw: smooth_regimes_causal() uses a lockout filter
         that only looks backwards.
      4. Macro publication lag is already handled upstream (2-month shift).
    """
    print("\n╔════════════════════════════════════════════════╗")
    print("  ║  WALK-FORWARD VALIDATION (strictly causal)      ║")
    print("  ╚════════════════════════════════════════════════╝")

    if df_sec is None or len(df_sec) < 100:
        from regime_detector_v2 import fetch_live_sector_data
    ...
    while fold_start < all_dates[-1]:
        ...
        # ── FIX 2: Causal anti-whipsaw filter (no lookahead smoothing) ──
        oos_labels = smooth_regimes_causal(oos_labels)
        ...
        rf_daily = 0.06 / 252
        for reg in ['Bull', 'Bear', 'HighVol', 'Sideways']:
            # Get training-period sector returns weighted by regime membership
            reg_mask = np.array([lab == reg for lab in train_labels])
            reg_dates_in_train = train_dates[reg_mask]
            common_reg = reg_dates_in_train.intersection(df_sec.index)

            if len(common_reg) < 30:
                # Not enough data → equal weight
                fold_sector_mix[reg] = {s: round(1.0/len(sector_names), 3) for s in sector_names}
                continue
...
def main():
    ...
    print("\n[7] Walk-forward validation (strictly causal — per-fold weights, online decode, pub-lag)...")
    # Note: learned_sector_mix is NOT passed — walk-forward learns per-fold (causal)
    folds_df, wf_daily_returns = walk_forward_validation(
        feat, df, df_sec=df_sec,
    ...
    print("\n[10] Live alert system & market status (100% real live model posteriors)...")
    ...
    if len(regime_changes) > 1:
        ...
        new_regime = result.loc[last_transition_date, 'Regime']
        print(f"  • Note: The last historical regime transition occurred on {last_transition_date.strftime('%d %b %Y')} ({prev_regime} ➜ {new_regime}).")
        print(f"  • Since {last_transition_date.strftime('%d %b %Y')}, the market has persisted in the {new_regime} regime through today ({result.index[-1].strftime('%d %b %Y')}).")
    ...
    sector_str = ""
    if learned_sector_mix and curr_reg in learned_sector_mix:
        top_s = sorted(learned_sector_mix[curr_reg].items(), key=lambda x: x[1], reverse=True)
        top_parts = [f"{s.replace('NIFTY ', '')}: {w*100:.1f}%" for s, w in top_s if w > 0.01]
        if top_parts:
            sector_str = " | ".join(top_parts)

    print("\n" + "=" * 65)
    print(f"🔔 LIVE MARKET STATUS — {latest_row.name.strftime('%d %b %Y')} (Latest Available Trading Day)")
    print("━" * 65)
    print(f"  Active Regime        : {curr_reg.upper()}")
    print(f"  NIFTY 50 Close       : {latest_row.get('NIFTY', 0):,.2f}")
